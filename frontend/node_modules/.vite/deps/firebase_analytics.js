import { Component, ErrorFactory, FirebaseError, Logger, _getProvider, _registerComponent, areCookiesEnabled, calculateBackoffMillis, deepEqual, getApp, getModularInstance, isBrowserExtension, isIndexedDBAvailable, openDB, registerVersion, validateIndexedDBOpenable } from "./index.esm-BS8WvbdL.js";
...
async function getInstallationEntry(installations) {
	...
	const installationEntry = await update(installations.appConfig, (oldEntry) => {
		const installationEntry$1 = updateOrCreateInstallationEntry(oldEntry);
		const entryWithPromise = triggerRegistrationIfNecessary(installations, installationEntry$1);
...
async function generateAuthTokenRequest({ appConfig, heartbeatServiceProvider }, installationEntry) {
	...
	if (response.ok) {
		const responseValue = await response.json();
		return extractAuthTokenInfoFromResponse(responseValue);
