...
import { require_jsx_runtime } from "./jsx-runtime-C18v7U6o.js";
...
function calcWH(positionParams, width, height, x, y, handle) {
	...
	if (handle === "sw" || handle === "w" || handle === "nw") _w = clamp(w, 0, cols);
	if (handle === "nw" || handle === "n" || handle === "ne") _h = clamp(h, 0, maxRows);
	return {
		w: _w,
		h: _h
	};
...
//#endregion
//#region node_modules/react-grid-layout/dist/chunk-FLITZMQC.mjs
...
function compactItemInternal(compareWith, l, compactType, cols, fullLayout, allowOverlap, b) {
	...
	while ((collision = getFirstCollision(compareWith, l)) !== void 0 && !allowOverlap) {
		if (compactH) resolveCompactionCollision(fullLayout, l, collision.x + collision.w, "x");
		else resolveCompactionCollision(fullLayout, l, collision.y + collision.h, "y");
...
function resolveCompactionCollision2(layout, item, moveToCoord, axis, hasStatics) {
	...
	for (let i = itemIndex + 1; i < layout.length; i++) {
		...
		if (otherItem.static) continue;
		if (!layoutHasStatics && otherItem.y > item.y + item.h) break;
		if (collides(item, otherItem)) resolveCompactionCollision2(layout, otherItem, moveToCoord + item[sizeProp], axis, layoutHasStatics);
	}
	item[axis] = moveToCoord;
}
function compactItemVertical(compareWith, l, fullLayout, maxY) {
	...
	while (l.y > 0 && !getFirstCollision(compareWith, l)) l.y--;
	let collision;
	while ((collision = getFirstCollision(compareWith, l)) !== void 0) resolveCompactionCollision2(fullLayout, l, collision.y + collision.h, "y");
	l.y = Math.max(l.y, 0);
	return l;
}
...
function getBreakpointFromWidth(breakpoints, width) {
	...
	for (let i = 1; i < sorted.length; i++) {
		...
		if (width > breakpointWidth) matching = breakpointName;
	}
	return matching;
}
...
var require_fast_equals = /* @__PURE__ */ __commonJS({ "node_modules/fast-equals/dist/fast-equals.js": ((exports, module) => {
	(function(global, factory) {
		...
		*/
		...
		function createComparator(_a) {
			var areArraysEqual$1 = _a.areArraysEqual, areDatesEqual$1 = _a.areDatesEqual, areMapsEqual$1 = _a.areMapsEqual, areObjectsEqual$1 = _a.areObjectsEqual, areRegExpsEqual$1 = _a.areRegExpsEqual, areSetsEqual$1 = _a.areSetsEqual, createIsNestedEqual = _a.createIsNestedEqual;
			...
			*/
			function comparator(a, b, meta) {
				...
				if (isPlainObject(a) && isPlainObject(b)) return areObjectsEqual$1(a, b, isEqual, meta);
				var aArray = Array.isArray(a);
				var bArray = Array.isArray(b);
				if (aArray || bArray) return aArray === bArray && areArraysEqual$1(a, b, isEqual, meta);
				var aTag = toString.call(a);
				if (aTag !== toString.call(b)) return false;
				if (aTag === DATE_TAG) return areDatesEqual$1(a, b, isEqual, meta);
				if (aTag === REG_EXP_TAG) return areRegExpsEqual$1(a, b, isEqual, meta);
				if (aTag === MAP_TAG) return areMapsEqual$1(a, b, isEqual, meta);
				if (aTag === SET_TAG) return areSetsEqual$1(a, b, isEqual, meta);
				if (aTag === OBJECT_TAG || aTag === ARGUMENTS_TAG) return isPromiseLike(a) || isPromiseLike(b) ? false : areObjectsEqual$1(a, b, isEqual, meta);
		...
		*/
		function areObjectsEqual(a, b, isEqual, meta) {
			...
			while (index-- > 0) {
				...
				if (!hasOwnProperty$1.call(b, key) || !isEqual(a[key], b[key], key, key, a, b, meta)) return false;
			}
			return true;
		}
		/**
		...
		*/
		...
		exports$1.deepEqual = deepEqual$2;
...
//#endregion
//#region node_modules/react-grid-layout/dist/chunk-LIKX67MZ.mjs
var import_react = /* @__PURE__ */ __toESM(require_react(), 1);
var import_fast_equals$1 = /* @__PURE__ */ __toESM(require_fast_equals(), 1);
...
function useGridLayout(options) {
	const { layout: propsLayout, cols, compactType = "vertical", allowOverlap = false, preventCollision = false, onLayoutChange } = options;
	const compactor = (0, import_react.useMemo)(() => getCompactor(compactType, allowOverlap), [compactType, allowOverlap]);
	...
	(0, import_react.useEffect)(() => {
		...
		if (!(0, import_fast_equals$1.deepEqual)(propsLayout, prevLayoutRef.current)) setLayout(propsLayout);
	}, [propsLayout, setLayout]);
	(0, import_react.useEffect)(() => {
		if (!(0, import_fast_equals$1.deepEqual)(layout, prevLayoutRef.current)) {
	...
	const onDrag = (0, import_react.useCallback)((itemId, x, y) => {
		...
		const newLayout = moveElement(layout, item, x, y, true, preventCollision, compactType, cols, allowOverlap);
	...
	const onResizeStop = (0, import_react.useCallback)((itemId, w, h) => {
		onResize(itemId, w, h);
		setResizeState({
			resizing: false,
			oldResizeItem: null,
	...
	const onDropDragOver = (0, import_react.useCallback)((droppingItem, position) => {
	...
	const onDrop = (0, import_react.useCallback)((droppingItem) => {
		const newLayout = layout.map((item) => {
...
function useResponsiveLayout(options) {
	const { width, breakpoints = DEFAULT_BREAKPOINTS, cols: colsConfig = DEFAULT_COLS, layouts: propsLayouts = {}, compactType = "vertical", onBreakpointChange, onLayoutChange, onWidthChange } = options;
	...
	const layout = (0, import_react.useMemo)(() => {
		return findOrGenerateResponsiveLayout(layouts, breakpoints, breakpoint, prevBreakpointRef.current, cols, compactType);
	}, [
		layouts,
		breakpoints,
		breakpoint,
	...
	(0, import_react.useEffect)(() => {
		...
		if (newBreakpoint !== breakpoint) {
			const newLayout = findOrGenerateResponsiveLayout(layouts, breakpoints, newBreakpoint, breakpoint, newCols, compactType);
	...
	(0, import_react.useEffect)(() => {
		if (!(0, import_fast_equals$1.deepEqual)(propsLayouts, prevPropsLayoutsRef.current)) {
			setLayouts(propsLayouts);
			prevPropsLayoutsRef.current = propsLayouts;
		}
	}, [propsLayouts, setLayouts]);
	(0, import_react.useEffect)(() => {
		if (!(0, import_fast_equals$1.deepEqual)(layouts, prevLayoutsRef.current)) {
...
var require_clsx = /* @__PURE__ */ __commonJS({ "node_modules/clsx/dist/clsx.js": ((exports, module) => {
	function r$1(e$1) {
		var o, t, f = "";
		if ("string" == typeof e$1 || "number" == typeof e$1) f += e$1;
		else if ("object" == typeof e$1) if (Array.isArray(e$1)) {
			var n = e$1.length;
			for (o = 0; o < n; o++) e$1[o] && (t = r$1(e$1[o])) && (f && (f += " "), f += t);
		} else for (t in e$1) e$1[t] && (f && (f += " "), f += t);
		return f;
	}
	function e() {
		for (var e$1, o, t = 0, f = "", n = arguments.length; t < n; t++) (e$1 = arguments[t]) && (o = r$1(e$1)) && (f && (f += " "), f += o);
		return f;
	}
...
var require_shims = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/utils/shims.js": ((exports) => {
...
var require_getPrefix = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/utils/getPrefix.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
...
var require_domFns = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/utils/domFns.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	var _shims$3 = require_shims();
	var _getPrefix = _interopRequireWildcard$4(require_getPrefix());
	function _interopRequireWildcard$4(e$1, t) {
		if ("function" == typeof WeakMap) var r$2 = /* @__PURE__ */ new WeakMap(), n = /* @__PURE__ */ new WeakMap();
		return (_interopRequireWildcard$4 = function(e$2, t$1) {
			if (!t$1 && e$2 && e$2.__esModule) return e$2;
			var o, i, f = {
				__proto__: null,
				default: e$2
			};
			if (null === e$2 || "object" != typeof e$2 && "function" != typeof e$2) return f;
			if (o = t$1 ? n : r$2) {
				if (o.has(e$2)) return o.get(e$2);
				o.set(e$2, f);
			}
			for (const t$2 in e$2) "default" !== t$2 && {}.hasOwnProperty.call(e$2, t$2) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e$2, t$2)) && (i.get || i.set) ? o(f, t$2, i) : f[t$2] = e$2[t$2]);
			return f;
		})(e$1, t);
	}
	var matchesSelectorFunc = "";
	function matchesSelector(el, selector) {
		if (!matchesSelectorFunc) matchesSelectorFunc = (0, _shims$3.findInArray)([
			"matches",
			...
			return (0, _shims$3.isFunction)(el[method]);
		});
		if (!(0, _shims$3.isFunction)(el[matchesSelectorFunc])) return false;
	...
	function outerHeight(node) {
		...
		height += (0, _shims$3.int)(computedStyle.borderTopWidth);
		height += (0, _shims$3.int)(computedStyle.borderBottomWidth);
		return height;
	}
	function outerWidth(node) {
		let width = node.clientWidth;
		const computedStyle = node.ownerDocument.defaultView.getComputedStyle(node);
		width += (0, _shims$3.int)(computedStyle.borderLeftWidth);
		width += (0, _shims$3.int)(computedStyle.borderRightWidth);
		return width;
	}
	function innerHeight(node) {
		let height = node.clientHeight;
		const computedStyle = node.ownerDocument.defaultView.getComputedStyle(node);
		height -= (0, _shims$3.int)(computedStyle.paddingTop);
		height -= (0, _shims$3.int)(computedStyle.paddingBottom);
		return height;
	}
	function innerWidth(node) {
		let width = node.clientWidth;
		const computedStyle = node.ownerDocument.defaultView.getComputedStyle(node);
		width -= (0, _shims$3.int)(computedStyle.paddingLeft);
		width -= (0, _shims$3.int)(computedStyle.paddingRight);
		return width;
	}
	...
	function getTouch(e$1, identifier) {
		return e$1.targetTouches && (0, _shims$3.findInArray)(e$1.targetTouches, (t) => identifier === t.identifier) || e$1.changedTouches && (0, _shims$3.findInArray)(e$1.changedTouches, (t) => identifier === t.identifier);
	}
	function getTouchIdentifier(e$1) {
		if (e$1.targetTouches && e$1.targetTouches[0]) return e$1.targetTouches[0].identifier;
		if (e$1.changedTouches && e$1.changedTouches[0]) return e$1.changedTouches[0].identifier;
	}
	...
	function addClassName(el, className) {
		...
		else if (!el.className.match(/* @__PURE__ */ new RegExp(`(?:^|\\s)${className}(?!\\S)`))) el.className += ` ${className}`;
	}
...
var require_positionFns = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/utils/positionFns.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	var _shims$2 = require_shims();
	var _domFns$2 = require_domFns();
	function getBoundPosition(draggable, x, y) {
		...
		if (typeof bounds === "string") {
			...
			bounds = {
				left: -node.offsetLeft + (0, _shims$2.int)(boundNodeStyle.paddingLeft) + (0, _shims$2.int)(nodeStyle.marginLeft),
				top: -node.offsetTop + (0, _shims$2.int)(boundNodeStyle.paddingTop) + (0, _shims$2.int)(nodeStyle.marginTop),
				right: (0, _domFns$2.innerWidth)(boundNodeEl) - (0, _domFns$2.outerWidth)(node) - node.offsetLeft + (0, _shims$2.int)(boundNodeStyle.paddingRight) - (0, _shims$2.int)(nodeStyle.marginRight),
				bottom: (0, _domFns$2.innerHeight)(boundNodeEl) - (0, _domFns$2.outerHeight)(node) - node.offsetTop + (0, _shims$2.int)(boundNodeStyle.paddingBottom) - (0, _shims$2.int)(nodeStyle.marginBottom)
			};
		}
		if ((0, _shims$2.isNum)(bounds.right)) x = Math.min(x, bounds.right);
		if ((0, _shims$2.isNum)(bounds.bottom)) y = Math.min(y, bounds.bottom);
		if ((0, _shims$2.isNum)(bounds.left)) x = Math.max(x, bounds.left);
		if ((0, _shims$2.isNum)(bounds.top)) y = Math.max(y, bounds.top);
		return [x, y];
	}
	...
	function getControlPosition(e$1, touchIdentifier, draggableCore) {
		const touchObj = typeof touchIdentifier === "number" ? (0, _domFns$2.getTouch)(e$1, touchIdentifier) : null;
		if (typeof touchIdentifier === "number" && !touchObj) return null;
		const node = findDOMNode(draggableCore);
		const offsetParent = draggableCore.props.offsetParent || node.offsetParent || node.ownerDocument.body;
		return (0, _domFns$2.offsetXYFromParent)(touchObj || e$1, offsetParent, draggableCore.props.scale);
	}
	function createCoreData(draggable, x, y) {
		const isStart = !(0, _shims$2.isNum)(draggable.lastX);
		const node = findDOMNode(draggable);
		if (isStart) return {
			node,
			deltaX: 0,
...
var require_log = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/utils/log.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.default = log;
	function log() {}
}) });
...
var require_DraggableCore = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/DraggableCore.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	var React$3 = _interopRequireWildcard$3(require_react());
	var _propTypes$4 = _interopRequireDefault$4(require_prop_types());
	var _reactDom$1 = _interopRequireDefault$4(require_react_dom());
	var _domFns$1 = require_domFns();
	var _positionFns$1 = require_positionFns();
	var _shims$1 = require_shims();
	var _log$1 = _interopRequireDefault$4(require_log());
	function _interopRequireDefault$4(e$1) {
		return e$1 && e$1.__esModule ? e$1 : { default: e$1 };
	}
	function _interopRequireWildcard$3(e$1, t) {
		if ("function" == typeof WeakMap) var r$2 = /* @__PURE__ */ new WeakMap(), n = /* @__PURE__ */ new WeakMap();
		return (_interopRequireWildcard$3 = function(e$2, t$1) {
			if (!t$1 && e$2 && e$2.__esModule) return e$2;
			var o, i, f = {
				__proto__: null,
				default: e$2
	...
	function _defineProperty$4(e$1, r$2, t) {
		return (r$2 = _toPropertyKey$4(r$2)) in e$1 ? Object.defineProperty(e$1, r$2, {
			value: t,
			enumerable: !0,
			configurable: !0,
			writable: !0
	...
	function _toPrimitive$4(t, r$2) {
		if ("object" != typeof t || !t) return t;
		var e$1 = t[Symbol.toPrimitive];
		if (void 0 !== e$1) {
			var i = e$1.call(t, r$2 || "default");
			if ("object" != typeof i) return i;
			throw new TypeError("@@toPrimitive must return a primitive value.");
		}
		return ("string" === r$2 ? String : Number)(t);
	}
	var eventsFor = {
		touch: {
			start: "touchstart",
	...
	var DraggableCore$2 = class extends React$3.Component {
		constructor() {
			super(...arguments);
			_defineProperty$4(this, "dragging", false);
			_defineProperty$4(this, "lastX", NaN);
			_defineProperty$4(this, "lastY", NaN);
			_defineProperty$4(this, "touchIdentifier", null);
			_defineProperty$4(this, "mounted", false);
			_defineProperty$4(this, "handleDragStart", (e$1) => {
				this.props.onMouseDown(e$1);
				if (!this.props.allowAnyClick && typeof e$1.button === "number" && e$1.button !== 0) return false;
				...
				if (this.props.disabled || !(e$1.target instanceof ownerDocument.defaultView.Node) || this.props.handle && !(0, _domFns$1.matchesSelectorAndParentsTo)(e$1.target, this.props.handle, thisNode) || this.props.cancel && (0, _domFns$1.matchesSelectorAndParentsTo)(e$1.target, this.props.cancel, thisNode)) return;
				if (e$1.type === "touchstart" && !this.props.allowMobileScroll) e$1.preventDefault();
				const touchIdentifier = (0, _domFns$1.getTouchIdentifier)(e$1);
				this.touchIdentifier = touchIdentifier;
				const position = (0, _positionFns$1.getControlPosition)(e$1, touchIdentifier, this);
				if (position == null) return;
				const { x, y } = position;
				const coreEvent = (0, _positionFns$1.createCoreData)(this, x, y);
				(0, _log$1.default)("DraggableCore: handleDragStart: %j", coreEvent);
				(0, _log$1.default)("calling", this.props.onStart);
				if (this.props.onStart(e$1, coreEvent) === false || this.mounted === false) return;
				if (this.props.enableUserSelectHack) (0, _domFns$1.addUserSelectStyles)(ownerDocument);
				this.dragging = true;
				this.lastX = x;
				this.lastY = y;
				(0, _domFns$1.addEvent)(ownerDocument, dragEventFor.move, this.handleDrag);
				(0, _domFns$1.addEvent)(ownerDocument, dragEventFor.stop, this.handleDragStop);
			});
			_defineProperty$4(this, "handleDrag", (e$1) => {
				const position = (0, _positionFns$1.getControlPosition)(e$1, this.touchIdentifier, this);
				if (position == null) return;
				let { x, y } = position;
				if (Array.isArray(this.props.grid)) {
					let deltaX = x - this.lastX, deltaY = y - this.lastY;
					[deltaX, deltaY] = (0, _positionFns$1.snapToGrid)(this.props.grid, deltaX, deltaY);
					if (!deltaX && !deltaY) return;
					x = this.lastX + deltaX, y = this.lastY + deltaY;
				}
				const coreEvent = (0, _positionFns$1.createCoreData)(this, x, y);
				(0, _log$1.default)("DraggableCore: handleDrag: %j", coreEvent);
				if (this.props.onDrag(e$1, coreEvent) === false || this.mounted === false) {
					try {
			...
			_defineProperty$4(this, "handleDragStop", (e$1) => {
				...
				const coreEvent = (0, _positionFns$1.createCoreData)(this, x, y);
				if (this.props.onStop(e$1, coreEvent) === false || this.mounted === false) return false;
				const thisNode = this.findDOMNode();
				if (thisNode) {
					if (this.props.enableUserSelectHack) (0, _domFns$1.scheduleRemoveUserSelectStyles)(thisNode.ownerDocument);
				}
				(0, _log$1.default)("DraggableCore: handleDragStop: %j", coreEvent);
				this.dragging = false;
				this.lastX = NaN;
				this.lastY = NaN;
				if (thisNode) {
					(0, _log$1.default)("DraggableCore: Removing handlers");
					(0, _domFns$1.removeEvent)(thisNode.ownerDocument, dragEventFor.move, this.handleDrag);
					(0, _domFns$1.removeEvent)(thisNode.ownerDocument, dragEventFor.stop, this.handleDragStop);
				}
			});
			_defineProperty$4(this, "onMouseDown", (e$1) => {
				dragEventFor = eventsFor.mouse;
				return this.handleDragStart(e$1);
			});
			_defineProperty$4(this, "onMouseUp", (e$1) => {
				dragEventFor = eventsFor.mouse;
				return this.handleDragStop(e$1);
			});
			_defineProperty$4(this, "onTouchStart", (e$1) => {
				dragEventFor = eventsFor.touch;
				return this.handleDragStart(e$1);
			});
			_defineProperty$4(this, "onTouchEnd", (e$1) => {
				dragEventFor = eventsFor.touch;
				return this.handleDragStop(e$1);
			});
		}
		componentDidMount() {
			this.mounted = true;
			const thisNode = this.findDOMNode();
			if (thisNode) (0, _domFns$1.addEvent)(thisNode, eventsFor.touch.start, this.onTouchStart, { passive: false });
		}
		componentWillUnmount() {
			...
			if (thisNode) {
				const { ownerDocument } = thisNode;
				(0, _domFns$1.removeEvent)(ownerDocument, eventsFor.mouse.move, this.handleDrag);
				(0, _domFns$1.removeEvent)(ownerDocument, eventsFor.touch.move, this.handleDrag);
				(0, _domFns$1.removeEvent)(ownerDocument, eventsFor.mouse.stop, this.handleDragStop);
				(0, _domFns$1.removeEvent)(ownerDocument, eventsFor.touch.stop, this.handleDragStop);
				(0, _domFns$1.removeEvent)(thisNode, eventsFor.touch.start, this.onTouchStart, { passive: false });
				if (this.props.enableUserSelectHack) (0, _domFns$1.scheduleRemoveUserSelectStyles)(ownerDocument);
			}
		}
		findDOMNode() {
		...
		render() {
			return /* @__PURE__ */ React$3.cloneElement(React$3.Children.only(this.props.children), {
	...
	exports.default = DraggableCore$2;
	_defineProperty$4(DraggableCore$2, "displayName", "DraggableCore");
	_defineProperty$4(DraggableCore$2, "propTypes", {
		allowAnyClick: _propTypes$4.default.bool,
		allowMobileScroll: _propTypes$4.default.bool,
		children: _propTypes$4.default.node.isRequired,
		disabled: _propTypes$4.default.bool,
		enableUserSelectHack: _propTypes$4.default.bool,
		offsetParent: function(props, propName) {
			if (props[propName] && props[propName].nodeType !== 1) throw new Error("Draggable's offsetParent must be a DOM Node.");
		},
		grid: _propTypes$4.default.arrayOf(_propTypes$4.default.number),
		handle: _propTypes$4.default.string,
		cancel: _propTypes$4.default.string,
		nodeRef: _propTypes$4.default.object,
		onStart: _propTypes$4.default.func,
		onDrag: _propTypes$4.default.func,
		onStop: _propTypes$4.default.func,
		onMouseDown: _propTypes$4.default.func,
		scale: _propTypes$4.default.number,
		className: _shims$1.dontSetMe,
		style: _shims$1.dontSetMe,
		transform: _shims$1.dontSetMe
	});
	_defineProperty$4(DraggableCore$2, "defaultProps", {
		allowAnyClick: false,
...
var require_Draggable = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/Draggable.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	var React$2 = _interopRequireWildcard$2(require_react());
	var _propTypes$3 = _interopRequireDefault$3(require_prop_types());
	var _reactDom = _interopRequireDefault$3(require_react_dom());
	var _clsx = require_clsx();
	var _domFns = require_domFns();
	var _positionFns = require_positionFns();
	var _shims = require_shims();
	var _DraggableCore = _interopRequireDefault$3(require_DraggableCore());
	var _log = _interopRequireDefault$3(require_log());
	function _interopRequireDefault$3(e$1) {
		return e$1 && e$1.__esModule ? e$1 : { default: e$1 };
	}
	function _interopRequireWildcard$2(e$1, t) {
		if ("function" == typeof WeakMap) var r$2 = /* @__PURE__ */ new WeakMap(), n = /* @__PURE__ */ new WeakMap();
		return (_interopRequireWildcard$2 = function(e$2, t$1) {
			if (!t$1 && e$2 && e$2.__esModule) return e$2;
			var o, i, f = {
				__proto__: null,
				default: e$2
	...
	function _extends$2() {
		return _extends$2 = Object.assign ? Object.assign.bind() : function(n) {
			for (var e$1 = 1; e$1 < arguments.length; e$1++) {
				var t = arguments[e$1];
				for (var r$2 in t) ({}).hasOwnProperty.call(t, r$2) && (n[r$2] = t[r$2]);
			}
			return n;
		}, _extends$2.apply(null, arguments);
	}
	function _defineProperty$3(e$1, r$2, t) {
		return (r$2 = _toPropertyKey$3(r$2)) in e$1 ? Object.defineProperty(e$1, r$2, {
			value: t,
			enumerable: !0,
			configurable: !0,
			writable: !0
	...
	var Draggable$1 = class extends React$2.Component {
		...
		constructor(props) {
			super(props);
			_defineProperty$3(this, "onDragStart", (e$1, coreData) => {
				(0, _log.default)("Draggable: onDragStart: %j", coreData);
				if (this.props.onStart(e$1, (0, _positionFns.createDraggableData)(this, coreData)) === false) return false;
				this.setState({
					dragging: true,
					dragged: true
				});
			});
			_defineProperty$3(this, "onDrag", (e$1, coreData) => {
				...
				if (this.props.onDrag(e$1, uiData) === false) return false;
				this.setState(newState);
			});
			_defineProperty$3(this, "onDragStop", (e$1, coreData) => {
				if (!this.state.dragging) return false;
				if (this.props.onStop(e$1, (0, _positionFns.createDraggableData)(this, coreData)) === false) return false;
		...
		render() {
			...
			return /* @__PURE__ */ React$2.createElement(_DraggableCore.default, _extends$2({}, draggableCoreProps, {
				onStart: this.onDragStart,
				onDrag: this.onDrag,
				onStop: this.onDragStop
			}), /* @__PURE__ */ React$2.cloneElement(React$2.Children.only(children), {
				className,
				style: {
	...
	exports.default = Draggable$1;
	_defineProperty$3(Draggable$1, "displayName", "Draggable");
	_defineProperty$3(Draggable$1, "propTypes", {
		..._DraggableCore.default.propTypes,
		axis: _propTypes$3.default.oneOf([
			"both",
			"x",
			"y",
			"none"
		]),
		bounds: _propTypes$3.default.oneOfType([
			_propTypes$3.default.shape({
				left: _propTypes$3.default.number,
				right: _propTypes$3.default.number,
				top: _propTypes$3.default.number,
				bottom: _propTypes$3.default.number
			}),
			_propTypes$3.default.string,
			_propTypes$3.default.oneOf([false])
		]),
		defaultClassName: _propTypes$3.default.string,
		defaultClassNameDragging: _propTypes$3.default.string,
		defaultClassNameDragged: _propTypes$3.default.string,
		defaultPosition: _propTypes$3.default.shape({
			x: _propTypes$3.default.number,
			y: _propTypes$3.default.number
		}),
		positionOffset: _propTypes$3.default.shape({
			x: _propTypes$3.default.oneOfType([_propTypes$3.default.number, _propTypes$3.default.string]),
			y: _propTypes$3.default.oneOfType([_propTypes$3.default.number, _propTypes$3.default.string])
		}),
		position: _propTypes$3.default.shape({
			x: _propTypes$3.default.number,
			y: _propTypes$3.default.number
		}),
	...
	_defineProperty$3(Draggable$1, "defaultProps", {
...
var require_cjs = /* @__PURE__ */ __commonJS({ "node_modules/react-draggable/build/cjs/cjs.js": ((exports, module) => {
	var { default: Draggable, DraggableCore: DraggableCore$1 } = require_Draggable();
	module.exports = Draggable;
	module.exports.default = Draggable;
	module.exports.DraggableCore = DraggableCore$1;
}) });

//#endregion
//#region node_modules/react-resizable/build/utils.js
var require_utils = /* @__PURE__ */ __commonJS({ "node_modules/react-resizable/build/utils.js": ((exports) => {
	exports.__esModule = true;
	exports.cloneElement = cloneElement;
	var _react = _interopRequireDefault$2(require_react());
	...
	function cloneElement(element, props) {
		if (props.style && element.props.style) props.style = _objectSpread$2(_objectSpread$2({}, element.props.style), props.style);
...
var require_propTypes = /* @__PURE__ */ __commonJS({ "node_modules/react-resizable/build/propTypes.js": ((exports) => {
	exports.__esModule = true;
	var _propTypes$2 = _interopRequireDefault$1(require_prop_types());
	require_cjs();
	function _interopRequireDefault$1(obj) {
		return obj && obj.__esModule ? obj : { default: obj };
	}
	var resizableProps = {
		axis: _propTypes$2.default.oneOf([
			"both",
			"x",
			"y",
			"none"
		...
		resizeHandles: _propTypes$2.default.arrayOf(_propTypes$2.default.oneOf([
			"s",
			"w",
			"e",
			"n",
...
var require_Resizable = /* @__PURE__ */ __commonJS({ "node_modules/react-resizable/build/Resizable.js": ((exports) => {
	exports.__esModule = true;
	var React$1 = _interopRequireWildcard$1(require_react());
	var _reactDraggable = require_cjs();
	var _utils = require_utils();
	var _propTypes$1 = require_propTypes();
	var _excluded$1 = [
		"children",
		"className",
		"draggableOpts",
	...
	function _getRequireWildcardCache$1(nodeInterop) {
		...
		return (_getRequireWildcardCache$1 = function _getRequireWildcardCache$2(nodeInterop$1) {
	...
	function _setPrototypeOf$1(o, p) {
		_setPrototypeOf$1 = Object.setPrototypeOf ? Object.setPrototypeOf.bind() : function _setPrototypeOf$2(o$1, p$1) {
			o$1.__proto__ = p$1;
			return o$1;
		};
	...
	var Resizable$1 = /* @__PURE__ */ function(_React$Component) {
		_inheritsLoose$1(Resizable$2, _React$Component);
		...
		var _proto = Resizable$2.prototype;
		...
		_proto.resizeHandler = function resizeHandler(handlerName, axis) {
			var _this2 = this;
			return function(e$1, _ref2) {
				var node = _ref2.node, deltaX = _ref2.deltaX, deltaY = _ref2.deltaY;
				if (handlerName === "onResizeStart") _this2.resetData();
				var canDragX$1 = (_this2.props.axis === "both" || _this2.props.axis === "x") && axis !== "n" && axis !== "s";
				var canDragY$1 = (_this2.props.axis === "both" || _this2.props.axis === "y") && axis !== "e" && axis !== "w";
				if (!canDragX$1 && !canDragY$1) return;
				var axisV = axis[0];
				...
				var width = _this2.props.width + (canDragX$1 ? deltaX / _this2.props.transformScale : 0);
				var height = _this2.props.height + (canDragY$1 ? deltaY / _this2.props.transformScale : 0);
		...
		_proto.renderResizeHandle = function renderResizeHandle(handleAxis, ref) {
			...
			var props = _objectSpread$1({ ref }, isDOMElement ? {} : { handleAxis });
...
var require_ResizableBox = /* @__PURE__ */ __commonJS({ "node_modules/react-resizable/build/ResizableBox.js": ((exports) => {
	exports.__esModule = true;
	...
	function _getRequireWildcardCache(nodeInterop) {
		...
		var cacheBabelInterop = /* @__PURE__ */ new WeakMap();
		var cacheNodeInterop = /* @__PURE__ */ new WeakMap();
		return (_getRequireWildcardCache = function _getRequireWildcardCache$2(nodeInterop$1) {
			return nodeInterop$1 ? cacheNodeInterop : cacheBabelInterop;
		})(nodeInterop);
	}
	...
	function _setPrototypeOf(o, p) {
		_setPrototypeOf = Object.setPrototypeOf ? Object.setPrototypeOf.bind() : function _setPrototypeOf$2(o$1, p$1) {
			o$1.__proto__ = p$1;
			return o$1;
		};
	...
	var ResizableBox = /* @__PURE__ */ function(_React$Component) {
		_inheritsLoose(ResizableBox$1, _React$Component);
		function ResizableBox$1() {
			var _this;
			for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) args[_key] = arguments[_key];
			_this = _React$Component.call.apply(_React$Component, [this].concat(args)) || this;
			_this.state = {
			...
			_this.onResize = function(e$1, data) {
				var size = data.size;
				if (_this.props.onResize) {
					e$1.persist == null || e$1.persist();
					_this.setState(size, function() {
						return _this.props.onResize && _this.props.onResize(e$1, data);
					});
				} else _this.setState(size);
			};
			return _this;
		}
		ResizableBox$1.getDerivedStateFromProps = function getDerivedStateFromProps(props, state) {
		...
		var _proto = ResizableBox$1.prototype;
		_proto.render = function render() {
...
var require_react_resizable = /* @__PURE__ */ __commonJS({ "node_modules/react-resizable/index.js": ((exports, module) => {
...
var import_react_resizable = /* @__PURE__ */ __toESM(require_react_resizable(), 1);
var import_cjs = /* @__PURE__ */ __toESM(require_cjs(), 1);
function r(e$1) {
	var t, f, n = "";
	if ("string" == typeof e$1 || "number" == typeof e$1) n += e$1;
	else if ("object" == typeof e$1) if (Array.isArray(e$1)) {
		var o = e$1.length;
		for (t = 0; t < o; t++) e$1[t] && (f = r(e$1[t])) && (n && (n += " "), n += f);
	} else for (f in e$1) e$1[f] && (n && (n += " "), n += f);
	return n;
}
function clsx() {
	for (var e$1, t, f = 0, n = "", o = arguments.length; f < o; f++) (e$1 = arguments[f]) && (t = r(e$1)) && (n && (n += " "), n += t);
	return n;
}
var clsx_default = clsx;

//#endregion
//#region node_modules/react-grid-layout/dist/chunk-PLWMASMX.mjs
var import_jsx_runtime = /* @__PURE__ */ __toESM(require_jsx_runtime(), 1);
var import_fast_equals = /* @__PURE__ */ __toESM(require_fast_equals(), 1);
function GridItem(props) {
	const { children, cols, containerWidth, margin, containerPadding, rowHeight, maxRows, isDraggable, isResizable, isBounded, static: isStatic, useCSSTransforms = true, usePercentages = false, transformScale = 1, droppingPosition, className = "", style, handle = "", cancel = "", x, y, w, h, minW = 1, maxW = Infinity, minH = 1, maxH = Infinity, i, resizeHandles, resizeHandle, constraints = defaultConstraints, layoutItem, layout = [], onDragStart: onDragStartProp, onDrag: onDragProp, onDragStop: onDragStopProp, onResizeStart: onResizeStartProp, onResize: onResizeProp, onResizeStop: onResizeStopProp } = props;
	...
	const onDragStart = (0, import_react.useCallback)((e$1, { node }) => {
		...
		const { x: newX, y: newY } = applyPositionConstraints(constraints, effectiveLayoutItem, rawPos.x, rawPos.y, constraintContext);
	...
	const onDrag = (0, import_react.useCallback)((e$1, { node, deltaX, deltaY }) => {
	...
	const onDragStop = (0, import_react.useCallback)((e$1, { node }) => {
	...
	const onResizeHandler = (0, import_react.useCallback)((e$1, { node, size, handle: resizeHandle2 }, position, handlerName) => {
		...
		const { w: newW, h: newH } = applySizeConstraints(constraints, effectiveLayoutItem, rawSize.w, rawSize.h, resizeHandle2, constraintContext);
		handler(i, newW, newH, {
			e: e$1.nativeEvent,
			node,
			size: updatedSize,
	...
	const handleResizeStart = (0, import_react.useCallback)((e$1, data) => {
		setResizing(true);
		...
		onResizeHandler(e$1, typedData, pos2, "onResizeStart");
	}, [
		onResizeHandler,
		positionParams,
		x,
	...
	const handleResize = (0, import_react.useCallback)((e$1, data) => {
		const pos2 = calcGridItemPosition(positionParams, x, y, w, h);
		const typedData = {
			...data,
			handle: data.handle
		};
		onResizeHandler(e$1, typedData, pos2, "onResize");
	}, [
		onResizeHandler,
		positionParams,
		x,
	...
	const handleResizeStop = (0, import_react.useCallback)((e$1, data) => {
		setResizing(false);
		...
		onResizeHandler(e$1, typedData, pos2, "onResizeStop");
	}, [
		onResizeHandler,
		positionParams,
		x,
...
function GridLayout(props) {
	...
	const { enabled: isDraggable, bounded: isBounded, handle: draggableHandle, cancel: draggableCancel } = dragConfig;
	...
	const [layout, setLayout] = (0, import_react.useState)(() => synchronizeLayoutWithChildren(propsLayout, children, cols, compactType, allowOverlap));
	...
	(0, import_react.useEffect)(() => {
		...
		if (layoutChanged || childrenChanged || compactTypeChanged) {
			const newLayout = synchronizeLayoutWithChildren(layoutChanged ? propsLayout : layout, children, cols, compactType, allowOverlap);
			setLayout(newLayout);
		}
	...
	const onResize = (0, import_react.useCallback)((i, w, h, data) => {
		...
		setLayout(allowOverlap ? finalLayout : compact(finalLayout, compactType, cols));
		setActiveDrag(placeholder);
	}, [
		preventCollision,
		allowOverlap,
	...
	const removeDroppingPlaceholder = (0, import_react.useCallback)(() => {
		const currentLayout = layoutRef.current;
		const newLayout = compact(currentLayout.filter((l) => l.i !== droppingItem.i), compactType, cols, allowOverlap);
		setLayout(newLayout);
	...
	const handleDragOver = (0, import_react.useCallback)((e$1) => {
		e$1.preventDefault();
		e$1.stopPropagation();
		if (isFirefox && !e$1.nativeEvent.target?.classList.contains(layoutClassName)) return false;
		const rawResult = onDropDragOverProp(e$1);
		if (rawResult === false) {
		...
		const gridRect = e$1.currentTarget.getBoundingClientRect();
		...
		const rawGridX = e$1.clientX - gridRect.left + dragOffsetX - itemCenterOffsetX;
		const rawGridY = e$1.clientY - gridRect.top + dragOffsetY - itemCenterOffsetY;
	...
	const handleDragLeave = (0, import_react.useCallback)((e$1) => {
		e$1.preventDefault();
		e$1.stopPropagation();
		dragEnterCounterRef.current--;
		if (dragEnterCounterRef.current === 0) removeDroppingPlaceholder();
	}, [removeDroppingPlaceholder]);
	const handleDragEnter = (0, import_react.useCallback)((e$1) => {
		e$1.preventDefault();
		e$1.stopPropagation();
		dragEnterCounterRef.current++;
	}, []);
	const handleDrop = (0, import_react.useCallback)((e$1) => {
		e$1.preventDefault();
		e$1.stopPropagation();
		const currentLayout = layoutRef.current;
		...
		removeDroppingPlaceholder();
		onDropProp(currentLayout, item, e$1.nativeEvent);
	}, [
		droppingItem.i,
		removeDroppingPlaceholder,
		onDropProp
...
function ResponsiveGridLayout(props) {
	...
	const derivedLayout = (0, import_react.useMemo)(() => {
		if (!(0, import_fast_equals.deepEqual)(propsLayouts, prevLayoutsRef.current)) return findOrGenerateResponsiveLayout(propsLayouts, breakpoints, breakpoint, breakpoint, cols, compactType);
		return null;
	}, [
		propsLayouts,
		breakpoints,
	...
	(0, import_react.useEffect)(() => {
		if (compactType !== prevCompactTypeRef.current) {
			const newLayout = compact(cloneLayout(effectiveLayout), compactType, cols, allowOverlap);
			const newLayouts = {
				...layoutsRef.current,
	...
	(0, import_react.useEffect)(() => {
		...
		if (widthChanged || breakpointPropChanged || breakpointsChanged || colsChanged) {
			...
			if (lastBreakpoint !== newBreakpoint || breakpointsChanged || colsChanged) {
				...
				let newLayout = findOrGenerateResponsiveLayout(newLayouts, breakpoints, newBreakpoint, lastBreakpoint, newCols, compactType);
				newLayout = synchronizeLayoutWithChildren2(newLayout, children, newCols, compactType, allowOverlap);
...
//#endregion
export { DEFAULT_BREAKPOINTS, DEFAULT_COLS, GridItem, GridLayout, GridLayout as ReactGridLayout, ResponsiveGridLayout as Responsive, ResponsiveGridLayout, bottom, calcGridItemPosition, calcWH, calcXY, cloneLayout, cloneLayoutItem, collides, GridLayout as default, findOrGenerateResponsiveLayout, getAllCollisions, getBreakpointFromWidth, getColsFromBreakpoint, getCompactor, getFirstCollision, getLayoutItem, horizontalCompactor, moveElement, noCompactor, setTopLeft, setTransform, sortLayoutItems, sortLayoutItemsByColRow, sortLayoutItemsByRowCol, useContainerWidth, useGridLayout, useResponsiveLayout, validateLayout, verticalCompactor };
