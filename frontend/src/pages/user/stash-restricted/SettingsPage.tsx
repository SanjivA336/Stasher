import { ScrollspyField, Spinner } from "@/components/Fields";
                ...
                {data.loadingContext ? (
                    ...
                    <>
                        <div className="w-1/5 flex items-center justify-center min-h-screen max-h-screen">
                            <ScrollspyField options={sections} offset={15} />
                        </div>
                        <div className="w-4/5 flex flex-col gap-6 ps-4 border-s-2 border-border" style={{ paddingTop: `${15}px`, paddingBottom: `${window.innerHeight / 1.4}px` }}>
                            <div id="stash-settings" className="w-full flex flex-col gap-4 max-h-screen border-border border-2 rounded-xl shadow-lg p-3">
                                <h1 className="text-2xl font-bold p-2 border-border border-b-2">Stash Settings</h1>
                                <StashSettings />
                            </div>
                            <div id="members-settings" className="w-full flex flex-col gap-4 max-h-screen border-border border-2 rounded-xl shadow-lg p-3">
                                <h1 className="text-2xl font-bold p-2 border-border border-b-2">Member Management</h1>
                                <MembersSettings />
                            </div>
                            <div id="storages-settings" className="w-full flex flex-col gap-4 max-h-screen border-border border-2 rounded-xl shadow-lg p-3">
                                <h1 className="text-2xl font-bold p-2 border-border border-b-2">Storage Management</h1>
                                <StoragesSettings />
                            </div>
                            <div id="labels-settings" className="w-full flex flex-col gap-4 max-h-screen border-border border-2 rounded-xl shadow-lg p-3">
